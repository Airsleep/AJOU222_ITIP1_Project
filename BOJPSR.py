...
def get_recommended_problem(input_target_user, similar_boundary):
    ...
    final_frequent_problems = get_frequent_items(shared_problem_dict_cnt)
    final_frequent_problems_list = list()
    for p in final_frequent_problems:
        final_frequent_problems_list.append(
            [p, boj_problems[p][1], boj_problems[p][2], boj_problems[p][3]])
    for fr in final_frequent_problems_list:
        print(fr)

    print("-"*10+" TOP5 RECOMMENDED PROBLEMS "+"-"*10)
    final_problem_dict_top5_list = []
    for t in final_problem_dict_top5:
        final_problem_dict_top5_list.append([t[0], t[1][0], t[1][1], t[1][2]])
    for t in final_problem_dict_top5_list:
        print(t)
